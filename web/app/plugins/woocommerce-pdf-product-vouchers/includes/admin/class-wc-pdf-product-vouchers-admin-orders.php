...
class WC_PDF_Product_Vouchers_Admin_Orders {
	...
	public function add_resend_voucher_recipient_email( $available_emails ) {
		...
		$order = isset( $_GET['post'] ) ? SV_WC_Plugin_Compatibility::wc_get_order( $_GET['post'] ) : null;
				...
				<tbody id="order_vouchers_list">
					...
					<?php if ( count( $order_items ) > 0 ) foreach ( $order_items as $item_id => $item ) :
						...
						if ( isset( $item['variation_id'] ) && $item['variation_id'] > 0 ) :
							$_product = SV_WC_Plugin_Compatibility::wc_get_product( $item['variation_id'] );
						else :
							$_product = SV_WC_Plugin_Compatibility::wc_get_product( $item['product_id'] );
						endif;
