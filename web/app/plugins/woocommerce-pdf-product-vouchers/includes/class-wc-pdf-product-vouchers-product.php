...
class WC_PDF_Product_Vouchers_Product {
	...
	public static function get_voucher( $product ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			foreach ( $product->get_children() as $variation_product_id ) {
				...
				$variation_product = SV_WC_Plugin_Compatibility::wc_get_product( $variation_product_id );

				if ( $variation_product->is_downloadable() && self::get_product_meta( $variation_product, 'voucher_id' ) ) {
					...
					return new WC_Voucher( self::get_product_meta( $variation_product, 'voucher_id' ) );
				}
			}
		} elseif ( $product->is_downloadable() && self::get_product_meta( $product, 'voucher_id' ) ) {
			// simple product or product variation
			return new WC_Voucher( self::get_product_meta( $product, 'voucher_id' ) );
		}
	...
	public static function get_voucher_id( $product ) {
		return self::get_product_meta( $product, 'voucher_id' );
	}


	/**
	 * Safely get the product meta named $field_name from $product, regardless
	 * of whether $product is a regular product or a product variation
	 *
	 * @since 1.2
	 * @param WC_Product $product the product or product variation
	 * @param string $field_name the field name to return
	 * @return mixed the value for $field_name or null
	 */
	public static function get_product_meta( $product, $field_name ) {
		...
		// WC 2.2 magic __get seems to only return parent meta for custom fields (core fields will work)
		if ( $product->is_type( 'variation' ) ) {
			$meta = get_post_meta( $product->variation_id, '_' . $field_name, true );
		}
